
/// <reference path="browser-sync-spa.d.ts" />
/// <reference path="gulp-autoprefixer.d.ts" />
/// <reference path="gulp-gh-pages.d.ts" />
/// <reference path="gulp-imagemin.d.ts" />
/// <reference path="gulp-replace-task.d.ts" />
/// <reference path="gulp-ruby-sass.d.ts" />
/// <reference path="gulp-scss-lint.d.ts" />
/// <reference path="http-proxy-middleware.d.ts" />
/// <reference path="jspm.d.ts" />
/// <reference path="karma.d.ts" />
/// <reference path="run-sequence.d.ts" />
/// <reference path="sw-precache.d.ts" />
/// <reference path="yargs.d.ts" />

